...
namespace FileCutter
{
    ...
    public class Process
    {
        ...
        public static void Main(string[] argv){
            bool cut=true;
            if(cut){
                Console.WriteLine("Start Cut...");
                var fileCutterProcess = new FileCutter.Process();
                //fileCutterProcess.Splitter(@"C:\Temp2\SQL2019-SSEI-Dev.exe", 2, SizeType.MBytes);
                fileCutterProcess.Splitter(@"C:\Temp2\SQLServer2019-x64-ENU-Dev.iso", 100, SizeType.MBytes);
            }
            else{
                Console.WriteLine("Start Join...");
                var fileCutterProcess = new FileCutter.Process();
                fileCutterProcess.Joiner(@"C:\Temp2");
            }
